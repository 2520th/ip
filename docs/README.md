...
1. randomly greets you with some surprises 😉
   greetings are adapted from [Monika After Story dlc of the Visual Novel Doki Doki Literature Club](https://github.com/Monika-After-Story/MonikaModDev/blob/master/Monika%20After%20Story/game/script-greetings.rpy)
3. asks you to set a username the first time you run this application (choose any non empty name besides Monika)
4. understands dates and time formats:
   ...
   Note: Times that are invalid or not in the correct format above will be stored in the original String's format. Monika will double check if you want to store this deadline / event with invalid, unsupported, or omitted time descriptions, answer "yes" to store the task with original String, "no" to discard this incorrect task. After this confirmation, omitted time description in deadlines or events will be displayed as "Unspecified". 
